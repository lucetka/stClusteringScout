...
def main():
   ...
   # I am using 50 as a hardcoded max becuase HDBSCAN doesn't work for hi-dim datasets
    ...
    if dimred_method == 'UMAP':
        ...
        st.sidebar.write("The 'highest granularity' level, i.e. eps = 0.00, will always be included in the results.")

        ### adding###
        mineps = st.sidebar.number_input(
                'Minimum cluster selection epsilon:',
                min_value=0.0, max_value=None, value=0.010, format="%.8f", key='mineps'
                    )
#### end addition###
        ...
        #renaming to maxeps, changing naming logic!!!!!! #####
        maxeps = st.sidebar.number_input('Maximum cluster selection epsilon:', min_value = 0.00000000, max_value = None, value =0.030 , format="%.8f", key = 'cluster_selection_epsilon' )   #, format="%f"
        ##end###
        ...
        #### CHANGE - because of changed logic above plus we mive it after the multiplier thing####
        #maxeps = cluster_selection_epsilon
        ## ###cluster_selection_epsilon = [0] + list(np.arange(mineps, maxeps+eps_step, eps_step))
        ##############
        ...
        if (st.sidebar.button("Create models with the selected hyperparameter ranges to compare")):   #if (st.sidebar.button("Create models with the selected hyperparameter ranges to compare")) & (epsstep <= maxeps):
            ...
            if (((mineps == int(mineps)) and (maxeps == int(maxeps)) and (epsstep == int(epsstep))) or (maxeps == 0)):
                ...
                if (epsstep <= 1):
                    ...
                    ### multiplier = int(np.power(10,(np.ceil(np.log10(maxeps)))))   CHANGE
                    multiplier = int(np.power(10,(np.ceil(np.log10(mineps)))))
            ...
            if epsstep != 0:
                for i in range(min_cluster_size_min,min_cluster_size_max+cluster_step, cluster_step):        
                    configurations[i] = {eps/multiplier: HDBSCAN(min_cluster_size = i,
                            ...
                            ###### now changing "for eps in range(0,int(multiplier*maxeps)+int(multiplier*epsstep),int(multiplier*(epsstep)))} " to start from mineps: ####
                                ### ADDED HERE THE [0] as explicit option
                            prediction_data=True) for eps in [0] + list(range(int(multiplier*mineps),int(multiplier*maxeps)+int(multiplier*epsstep),int(multiplier*(epsstep))))}   #(0, maxeps+epsstep, epsstep)   } #np.linspace(0,maxeps,int(maxeps/epsstep))}
...
###################################################
            ...
            # 
            #    
                    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs([ "2D plots", "Cluster_selection_epsilon range recommendations", "Summary plot", "Recommendations", "Clusterings vs hyperparameters", "Diagnostic plots", "Models in use", "Download results"])
                    ...
                    with tab8:
                        ...
                        # --- Build export table (prefer using your cached helper if you added it) ---
                        # If you already defined @st.cache_data build_export_table(...) at top-level, use it:
                        try:
                            export_df = build_export_table(results, data, number_dimensions, min_samples)
                        except NameError:
                            # Fallback: inline builder (uncached)
                            export_df = pd.DataFrame({"ID": data.index})
                            for nn in results.keys():
                                for mcs in results[nn].keys():
                                    for eps, labels in results[nn][mcs].items():
                        ...
                        # --- EPS recommendations (raw + formatted, as in Tab 2) ---
                        try:
                            eps_ranges_df = pd.DataFrame(useful_eps_ranges)
                            eps_ranges_df_formatted = eps_ranges_df.add_prefix('n_neighbors(UMAP)_')
                            eps_ranges_df_formatted.index.names = ['Min cluster size']
                        except Exception as _e:
                            eps_ranges_df = None
                            eps_ranges_df_formatted = None
                            st.info(f"EPS recommendations not available: {str(_e)}")

                        # --- Safe helpers (HTML/CSV) ---
                        ...
                        def to_csv_safe(df, index=False):
                            try:
                                return df.to_csv(index=index)
                            except Exception:
                                return None

                        # --- Try to (re)create the 2D plots here to include in ZIP ---
                        # Using your 'vizred' (2D embedding for min/max n_neighbors)
                        try:
                            nn_min = st.session_state.n_neighbors_min[0]
                            nn_max = st.session_state.n_neighbors_max[0]
                            fig_1 = px.scatter(
                                data, x=vizred[nn_min][:, 0], y=vizred[nn_min][:, 1],
                                hover_data={data.index.name: (data.index)},
                                title=f'n_neighbors {nn_min}'
                            )
                            fig_2 = px.scatter(
                                data, x=vizred[nn_max][:, 0], y=vizred[nn_max][:, 1],
                                hover_data={data.index.name: (data.index)},
                                title=f'n_neighbors {nn_max}'
                            )
                        except Exception:
                            fig_1 = None
                            fig_2 = None
                        ...
                        with zipfile.ZipFile(buffer, "w") as z:
                            ...
                            # EPS recommendations
                            if eps_ranges_df is not None:
                                raw_csv = to_csv_safe(eps_ranges_df, index=True)
                                if raw_csv:
                                    z.writestr("eps_recommendations_raw.csv", raw_csv)
                            if eps_ranges_df_formatted is not None:
                                fmt_csv = to_csv_safe(eps_ranges_df_formatted, index=True)
                                if fmt_csv:
                            ...
                            # Clusterings vs hyperparameters plots (Tab 5)
...
###################################################ending addition from copilot 1-dec-25
                #### end added after demo
